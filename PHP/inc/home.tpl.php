<main class="min-vh-100 min-vw-100 d-flex justify-content-center align-items-center">
    <div class="container-fluid min-vh-100 p-0 m-0 d-flex align-items-center justify-content-center flex-column">
        <div class="row d-flex align-items-center flex-column">
            <div class="col-5 text-center mt-3">
                <img src="../images/moi.png" alt="photo de Christophe LE LAZ" class="w-100 h-100">
            </div>
            <section class="col-7 text-center w-100">
                <h6 class="p-2">Bonjour, je suis</h6>
                <h2 class="">Christophe</h2>
                <h2 class="p-2">Le Laz</h2>
                <h4 class="">Développeur Web en formation</h3>
            </section>
        </div>
        <div class="row mt-5">
            <section class="col d-flex flex-column align-items-center">
                <h5>Choisissez votre thème</h5>
                <div class="content">
                    <div class="content__container position-relative">
                    <div class="content__container__before">[</div>
                        <ul class="content__container__list text-center">
                            <li class="content__container__list__item"><a href="index.php?page=classique">classique</a></li>
                            <li class="content__container__list__item"><a href="index.php?page=sportif">sportif</a></li>
                            <li class="content__container__list__item"><a href="index.php?page=fun">fun</a></li>
                            <li class="content__container__list__item"><a href="" id="aleatoireTheme">aléatoire</a></li>
                        </ul>
                        <div class="content__container__after">]</div>

                    </div>
                </div>
            </section>
        </div>
    </div>